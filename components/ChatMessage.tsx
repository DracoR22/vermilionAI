    ...
    <div className={cn('group flex items-start gap-x-3 py-4 w-full',
     role === 'user' && 'justify-end')}>
       ...
       <div className="rounded-md px-4 py-2 max-w-sm text-sm bg-primary/10">
         {isLoading ? <SyncLoader size={5} color={theme === 'light' ? 'black' : 'white'}/> : content}
       </div>
       {role === 'user' && <UserAvatar/>}
       {role !== 'user' && !isLoading && (
        <Button onClick={onCopy} className="opacity-0 group-hover:opacity-100 transition" variant='ghost'>
